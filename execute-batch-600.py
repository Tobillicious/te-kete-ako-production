#!/usr/bin/env python3
"""
Execute Batch 600 Insertions - Add Next 50 Documents to GraphRAG
Executes all SQL insertions from batch_600_insertions.sql

Current Status: 1,079 entries
Target: Execute 50 insertions to reach ~1,129 entries
"""
...
def execute_batch_insertions():
    ...
    sql_file = Path('batch_600_insertions.sql')

    if not sql_file.exists():
        print("❌ No SQL file found. Run batch-600-indexer.py first.")
        return False
    ...
    if failed_inserts == 0:
        ...
        print("\n📊 NEW GRAPHRAG STATUS:")
        print(f"   - Total entries: {1079 + successful_inserts}")
        print(f"   - Coverage increase: +{successful_inserts} documents")
        print(f"   - Knowledge gap reduced to: {((1412 - (1079 + successful_inserts)) / 1412) * 100:.1f}%")
        return True
    else:
...
def main():
    ...
    print("🧠 EXECUTING BATCH 600 INSERTIONS - Knowledge Gap Closure")
    print("=" * 60)
