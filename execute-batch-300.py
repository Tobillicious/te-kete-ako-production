#!/usr/bin/env python3
"""
Execute Batch 300 Insertions - Add Next 50 Documents to GraphRAG
Executes all SQL insertions from batch_300_insertions.sql

Current Status: 929 entries
Target: Execute 50 insertions to reach ~979 entries
"""
...
def execute_batch_insertions():
    ...
    if not sql_file.exists():
        print("❌ No SQL file found. Run batch-300-indexer.py first.")
        return False
    ...
    if failed_inserts == 0:
        ...
        print("\n📊 NEW GRAPHRAG STATUS:")
        print(f"   - Total entries: {929 + successful_inserts}")
        print(f"   - Coverage increase: +{successful_inserts} documents")
        print(f"   - Knowledge gap reduced to: {((1412 - (929 + successful_inserts)) / 1412) * 100:.1f}%")
        return True
    else:
...
def main():
    ...
    print("🧠 EXECUTING BATCH 300 INSERTIONS - Knowledge Gap Closure")
    print("=" * 60)
...
🎊 BATCH INSERTION COMPLETE!"        print("   - GraphRAG knowledge base significantly expanded")
        ...
        print("
🎯 RECOMMENDED NEXT STEPS:"        print("   - Process next 50 priority files")
