#!/usr/bin/env python3
"""
Execute Batch 200 Insertions - Add Next 50 Documents to GraphRAG
Executes all SQL insertions from batch_200_insertions.sql

Current Status: 879 entries
Target: Execute 50 insertions to reach ~929 entries
"""
...
def execute_batch_insertions():
    ...
    sql_file = Path('batch_200_insertions.sql')

    if not sql_file.exists():
        print("❌ No SQL file found. Run batch-200-indexer.py first.")
        return False
    ...
    if failed_inserts == 0:
        ...
        print("\n📊 NEW GRAPHRAG STATUS:")
        print(f"   - Total entries: {879 + successful_inserts}")
        print(f"   - Coverage increase: +{successful_inserts} documents")
        print(f"   - Knowledge gap reduced to: {((1412 - (879 + successful_inserts)) / 1412) * 100:.1f}%")
        return True
    else:
...
def main():
    ...
    print("🧠 EXECUTING BATCH 200 INSERTIONS - Knowledge Gap Closure")
    print("=" * 60)
    ...
    if success:
        ...
        print("   4. Complete remaining files systematically")
    else:
